...
export default async function AdminDashboardPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-10">
    ...
    {attention.length ? <div className="mt-5 grid gap-2">{attention.map((item) => <Link className="rounded-xl border border-wayne-warn/40 bg-wayne-warn-soft px-4 py-3 font-bold" href={item.href} key={item.text}>{item.text} →</Link>)}</div> : null}
...
function DashboardBody({ dashboard, preset, tipsEnabled }: { dashboard: Dashboard; preset: string; tipsEnabled: boolean }) {
  ...
  return <>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_18rem]">
      ...
      <Card className="p-5"><h2 className="text-lg font-black">Recent orders</h2>{dashboard.recent_orders.length ? <ul className="mt-3 grid gap-2">{dashboard.recent_orders.map((order) => <li className="flex justify-between gap-3" key={order.id}><Link className="underline" href={`/admin/orders/${order.id}`}>{order.order_number} · {order.customer_name}</Link><span className={`whitespace-nowrap text-sm font-bold ${order.status === "cancelled" ? "text-wayne-alert line-through" : ""}`}>{titleCase(order.status)} · {formatCents(order.total_cents)}</span></li>)}</ul> : <Empty />}</Card>
...
function Mix({ title, rows }: { title: string; rows: Array<[string, { order_count: number; total_cents: number }]> }) {
  ...
  return <Card className="p-5"><h2 className="text-lg font-black">{title}</h2><dl className="mt-3 grid gap-3">{rows.map(([label, row]) => <div key={label}><div className="flex justify-between gap-3"><dt>{label} <span className="text-sm text-wayne-muted">· {row.order_count} order{row.order_count === 1 ? "" : "s"}</span></dt><dd className="font-bold tabular-nums">{formatCents(row.total_cents)}</dd></div><div aria-hidden="true" className="mt-1 h-1.5 rounded-full bg-wayne-cream-deep"><div className="h-1.5 rounded-full bg-wayne-red" style={{ width: `${total ? Math.round((row.total_cents / total) * 100) : 0}%` }} /></div></div>)}</dl></Card>;
}
...
function HourChart({ title, rows, format }: { title: string; rows: Array<{ hour: number; value: number }>; format: (value: number) => string }) {
  ...
  return <Card className="p-5"><h2 className="text-lg font-black">{title}</h2>
    {max === 0 ? <Empty /> : <>
      <div className="mt-4 flex h-40 items-end gap-0.5 border-b border-wayne-border-strong" role="img" aria-label={`${title}: busiest hour ${label(rows.reduce((best, row) => row.value > best.value ? row : best).hour)}`}>
