...
export default async function AdminCalendarPage({ searchParams }: { searchParams: Promise<{ month?: string | string[] }> }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-10">
      ...
      <div className="mt-5 hidden overflow-hidden rounded-2xl border border-wayne-border bg-white sm:block"><div className="grid grid-cols-7 bg-wayne-ink text-center text-xs font-black uppercase tracking-wide text-white">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div className="p-3" key={day}>{day}</div>)}</div><div className="grid grid-cols-7">{cells.map((cell) => { const day = byDate.get(cell.date); return <Link aria-current={cell.date === today ? "date" : undefined} aria-label={`View orders for ${cell.date}`} className={`min-h-32 border-r border-t border-wayne-border p-3 transition hover:bg-wayne-cream ${cell.inMonth ? "" : "bg-wayne-cream text-wayne-cream/60"} ${cell.date === today ? "bg-wayne-warn-soft ring-2 ring-inset ring-wayne-red" : ""}`} href={`/admin/orders?from=${cell.date}&to=${cell.date}`} key={cell.date}><strong>{cell.day}{cell.date === today ? <span className="ml-2 rounded-full bg-wayne-red px-2 py-1 text-xs text-white">Today</span> : null}</strong>{day ? <div className="mt-4"><p className="font-black text-wayne-red">{day.order_count} order{day.order_count === 1 ? "" : "s"}</p><p className="text-sm font-bold">{formatCents(day.active_total_cents)}</p><p className="mt-1 text-xs text-wayne-muted">{day.pickup_count} pickup · {day.delivery_count} delivery</p></div> : <p className="mt-4 text-xs text-wayne-muted">No orders</p>}</Link>; })}</div></div>
      <div className="mt-5 grid gap-3 sm:hidden">{cells.filter((cell) => cell.inMonth).map((cell) => { const day = byDate.get(cell.date); const isToday = cell.date === today; return <Link aria-current={isToday ? "date" : undefined} aria-label={`View orders for ${cell.date}${isToday ? ", today" : ""}`} href={`/admin/orders?from=${cell.date}&to=${cell.date}`} key={cell.date}><Card className={`flex min-h-20 items-center justify-between gap-4 p-4 ${isToday ? "border-wayne-red bg-wayne-warn-soft ring-2 ring-inset ring-wayne-red" : ""}`}><div><strong>{new Intl.DateTimeFormat("en-US", { weekday: "short", month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(`${cell.date}T00:00:00Z`))}{isToday ? <span className="ml-2 rounded-full bg-wayne-red px-2 py-1 text-xs text-white">Today</span> : null}</strong><p className="text-sm text-wayne-muted">{day ? `${day.pickup_count} pickup · ${day.delivery_count} delivery` : "No orders"}</p></div><div className="text-right"><strong className="text-wayne-red">{day?.order_count ?? 0}</strong><p className="text-sm font-bold">{formatCents(day?.active_total_cents ?? 0)}</p></div></Card></Link>; })}</div>
    </main>
  );
}
