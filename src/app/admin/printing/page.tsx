...
export const metadata: Metadata = { title: "Kitchen print queue" };
...
export default async function PrintingPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string; view?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-10"><p className="text-sm font-black uppercase tracking-widest text-wayne-red">Kitchen operations</p><h1 className="mt-3 text-4xl font-black">Print queue</h1>
    <p className="mt-3 max-w-3xl text-wayne-muted">Tickets remain saved here while a printer is unavailable. Printing needs a connected print agent and a compatible printer adapter.</p>
    ...
    {params.saved ? <p role="status" className="mt-5 rounded-xl bg-green-50 p-4">Job returned to the pending queue. Retry reason recorded.</p> : null}
    {params.error || error ? <p role="alert" className="mt-5 rounded-xl bg-red-50 p-4">{params.error || "Print queue unavailable. Check the database migrations and connection."}</p> : null}
    <div className="mt-6 grid gap-4">{jobs?.map((job) => {
      ...
      return <Card className="p-5" key={job.id}><div className="flex flex-wrap justify-between gap-4"><div><Link className="text-xl font-black underline" href={`/admin/orders/${job.order_id}`}>{orderNumber}</Link><p>{job.destination} · {job.attempts} attempt{job.attempts === 1 ? "" : "s"}</p></div><strong className="capitalize">{expired ? "Needs inspection — expired lease" : job.status}</strong></div>
      {job.last_error ? <p className="mt-3 text-sm text-red-800">Last error: {job.last_error}</p> : null}
