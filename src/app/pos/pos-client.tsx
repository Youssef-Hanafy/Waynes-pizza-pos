      ...
      <div className="overflow-y-auto p-5">
        ...
        {item.modifier_groups.length ? item.modifier_groups.map((group) => <fieldset className="mt-5" key={group.id}><legend className="font-black">{group.customer_label} <span className="font-normal text-wayne-muted">({group.min_select}–{group.max_select})</span></legend><div className="mt-2 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{group.choices.map((choice) => {
          ...
          return <div className={`grid min-h-16 grid-cols-[3.25rem_1fr_3.25rem] overflow-hidden rounded-xl border ${count ? "border-wayne-ok bg-wayne-ok/10" : "border-wayne-border bg-white"}`} key={choice.id}>
            ...
            <div className="flex min-w-0 flex-col justify-center px-3"><strong className="truncate">{choice.name}</strong><span className="text-xs text-wayne-muted">{count ? `${count} portion${count === 1 ? "" : "s"}` : included ? "included" : "not selected"}{choice.price_delta_cents ? ` · +${formatCents(choice.price_delta_cents)} each` : ""}</span></div>
...
/* Legacy checkbox customizer retained here temporarily for diff context.
function ItemDialog({ initialLine, item, onAdd, onClose }: { initialLine: CartLine | null; item: MenuItem; onAdd: (line: CartLine) => void; onClose: () => void }) {
  ...
  return <div aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-3" role="dialog"><section className="max-h-[95vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl"><div className="flex justify-between gap-4"><div><h2 className="text-3xl font-black">{item.name}</h2><p className="text-wayne-muted">{item.description}</p></div><button aria-label="Close item" className="h-12 w-12 rounded-full border text-xl" onClick={onClose}>×</button></div>{item.variants.length ? <fieldset className="mt-5"><legend className="font-black">Size</legend><div className="mt-2 grid grid-cols-2 gap-2">{item.variants.map((variant) => <label className="flex min-h-14 items-center justify-between rounded-xl border p-3" key={variant.id}><span><input checked={variantId === variant.id} className="mr-2" name="variant" onChange={() => setVariantId(variant.id)} type="radio" />{variant.name}</span><strong>{formatCents(variant.price_cents)}</strong></label>)}</div></fieldset> : null}{item.modifier_groups.map((group) => <fieldset className="mt-5" key={group.id}><legend className="font-black">{group.customer_label} <span className="font-normal text-wayne-muted">({group.min_select}–{group.max_select})</span></legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{group.choices.map((choice) => { const count = selectedChoices[choice.id] ?? 0; const delta = choicePriceDeltaCents(choice, variantId); return <div className="flex min-h-14 items-center rounded-xl border p-3" key={choice.id}><label className="flex flex-1 items-center"><input checked={count > 0} className="mr-2" onChange={(e) => setSelectedChoices({ ...selectedChoices, [choice.id]: e.target.checked ? 1 : 0 })} type="checkbox" />{choice.name} {delta ? <small className="ml-1">{delta > 0 ? "+" : ""}{formatCents(delta)}</small> : null}</label>{group.allow_quantities && count ? <div className="flex items-center gap-2"><button className="h-9 w-9 rounded border" onClick={() => setSelectedChoices({ ...selectedChoices, [choice.id]: Math.max(0, count - 1) })}>−</button><strong>{count}</strong><button className="h-9 w-9 rounded border" onClick={() => setSelectedChoices({ ...selectedChoices, [choice.id]: Math.min(20, count + 1) })}>+</button></div> : null}</div>; })}</div></fieldset>)}<label className="mt-5 grid gap-2 font-bold">Item notes<textarea className="rounded-xl border p-3 font-normal" maxLength={500} onChange={(e) => setInstructions(e.target.value)} rows={2} value={instructions} /></label><div className="mt-5 flex items-center justify-between gap-4"><div className="flex items-center gap-2"><button className="h-12 w-12 rounded-xl border" onClick={() => setQuantity(Math.max(1, quantity - 1))}>−</button><strong>{quantity}</strong><button className="h-12 w-12 rounded-xl border" onClick={() => setQuantity(Math.min(20, quantity + 1))}>+</button></div><Button className="min-h-14" onClick={add}>{initialLine ? "Save changes" : "Add"} · {formatCents(cartLineUnitCents(draftMenu, draftLine) * quantity)}</Button></div>{error ? <p className="mt-3 rounded-lg bg-wayne-alert-soft p-3 font-bold text-wayne-alert">{error}</p> : null}</section></div>;
}
*/
