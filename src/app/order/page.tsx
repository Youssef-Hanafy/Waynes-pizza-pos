...
export default async function OrderPage() {
  ...
  const open = isStoreOpenNow(settings) && settings.test_ordering_enabled;
  return <div className="min-h-screen bg-wayne-cream"><SiteHeader settings={settings} /><main className="mx-auto max-w-5xl px-5 py-16 text-center sm:py-24"><p className="text-sm font-black uppercase tracking-[0.2em] text-wayne-red">Order Wayne&apos;s Pizza</p><h1 className="mt-4 text-5xl font-black sm:text-6xl">How can we make it?</h1><p className="mx-auto mt-5 max-w-xl text-lg text-wayne-muted">Choose pickup or delivery. You can change this before checkout.</p>{!open ? <p className="mx-auto mt-7 max-w-xl rounded-xl border border-amber-300 bg-amber-50 p-4 font-bold">Online ordering is currently closed. Please call {settings.public_phone || "Wayne's Pizza"} for help.</p> : null}<div className="mt-10 grid gap-5 sm:grid-cols-2"><OrderOption disabled={!open || !settings.pickup_enabled} href="/menu?fulfillment=pickup" label="Pickup" description={settings.pickup_description} /><OrderOption disabled={!open || !settings.delivery_enabled} href="/menu?fulfillment=delivery" label="Delivery" description={settings.delivery_description} /></div></main><SiteFooter settings={settings} /></div>;
}
