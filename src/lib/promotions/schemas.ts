...
export const promotionSchema = z.object({
  ...
  total_usage_limit: z.number().int().nullable(), per_customer_limit: z.number().int().nullable(), uses_count: z.number().int(), active: z.boolean(), archived_at: z.string().nullable(), created_at: z.string(),
});
...
export function parsePromotionForm(values: Record<string, FormDataEntryValue | null>, timeZone: string) {
  const parsed = z.object({
    ...
    active: z.boolean(),
  }).safeParse({ ...values, active: values.active === "on", description: values.description ?? "", starts_at: values.starts_at ?? "", ends_at: values.ends_at ?? "", fulfillment_type: values.fulfillment_type ?? "" });
