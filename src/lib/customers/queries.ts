...
export async function getCustomers(filters: { search?: string; segmentId?: string; page?: number }) { const supabase = await createServerSupabaseClient(); const { data, error } = await supabase.rpc("wayne_admin_customers", { search_text: filters.search || null, segment_filter: filters.segmentId || null, page_size: 50, page_offset: Math.max(0, ((filters.page ?? 1) - 1) * 50) }); if (error) throw new Error(`Customer search failed: ${error.message}`); return customerListSchema.parse(data); }
